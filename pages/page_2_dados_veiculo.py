...
def render(t):
    ...
    if not st.session_state.data_loaded:
        st.warning(t("error_no_file"), icon=":material/warning:")
        return
    ...
    with st.container(border=True):
        ...
        st.session_state.vehicle_info["test_date"] = (
            st.session_state.test_date_input
        )

    st.space("small")

    with st.container(border=True):
        st.subheader(f":material/balance: {t('split_vehicle_mass_title')}")
        ...
        with st.container(horizontal=True, gap="medium"):
            running_order_mass = st.number_input(
                t("split_running_order_mass"),
                min_value=0.0,
                max_value=50000.0,
                value=float(running_default),
                step=10.0,
                format="%.2f",
            )
            with st.container():
                ...
                if rotational_available:
                    rotational_mass = st.number_input(
                        ...
                        min_value=0.0,
                        max_value=5000.0,
                        value=float(rotational_default or 1.0),
                        step=1.0,
                        format="%.2f",
                    )
                else:
        ...
        if running_order_mass <= 0:
            ...
            st.warning(t("error_no_mass"), icon=":material/warning:")
            return
        ...
        st.session_state.vehicle_info["rotational_mass_available"] = (
            rotational_available
        )
        _store_mass_data(mass_data)
        _invalidate_mass_dependent_results(
            previous_effective_mass,
            mass_data["effective_mass_kg"],
        )
        ...
        with st.container(horizontal=True, gap="small"):
            st.metric(
                f":material/scale: {t('split_test_mass')}",
                f"{mass_data['test_mass_kg']:.2f} kg",
                border=True,
            )
            st.metric(
                f":material/sync: {t('split_rotational_equivalent_mass')}",
                f"{mass_data['rotational_equivalent_mass_kg']:.2f} kg",
                border=True,
            )
            st.metric(
                f":blue[:material/calculate: {t('split_effective_mass')}]",
                f"{mass_data['effective_mass_kg']:.2f} kg",
                border=True,
            )

    st.space("small")
    if st.button(
        t("split_confirm_vehicle_data"),
        type="primary",
        width="stretch",
    ):
        st.session_state.vehicle_data_complete = True
        st.success(t("split_vehicle_data_ready"), icon=":material/check_circle:")
